  ...
  return (
    <section>
      ...
      <div style={{display:'grid', gap:16}}>
        {days.map((d, i) => <DayLog key={i} day={d.day} segments={d.segments}/>)}
      </div>
    </section>
  )
}

function DayLog({day, segments}){
  let t = 0
  const lines = []
  segments.forEach((s, idx) => {
    ...
    const x2 = x1 + w
    lines.push(<line key={idx} x1={x1} y1={rowY} x2={x2} y2={rowY} strokeWidth="6" />)
    t += (s.hours || 0)
  })

  return (
    <div style={{overflowX:'auto', border:'1px solid #eee', borderRadius:8, padding:12}}>
      <svg width={GRID_W+80} height={GRID_H+40}>
        <text x="10" y="18" fontSize="14" fontWeight="bold">Day {day}</text>
        {ROWS.map(r => (
          <g key={r.key}>
        ...
        {[...Array(25)].map((_,h)=>(
          <g key={h}>
            ...
            {h<24 && <text x={X0+h*HOUR_W+4} y={GRID_H+20} fontSize="10">{h}:00</text>}
          </g>
        ))}
        {lines}
      </svg>
